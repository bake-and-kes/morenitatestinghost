...
function verificarEstadoCatalogo() {
    return new Promise((resolve, reject) => {
        database.ref('config/catalogoEspecial').once('value').then(snapshot => {
            const estaAbierto = snapshot.val() === true;
            const overlay = document.getElementById('catalogoCerradoOverlay');
            
            console.log('Estado del catálogo:', estaAbierto ? 'ABIERTO' : 'CERRADO');
            
            if (overlay) {
                if (estaAbierto) {
                    // Catálogo abierto = ocultar overlay
                    overlay.style.display = 'none';
                    console.log('Overlay ocultado - catálogo disponible');
                } else {
                    // Catálogo cerrado = mostrar overlay
                    overlay.style.display = 'flex';
                    console.log('Overlay mostrado - catálogo cerrado');
                }
            } else {
                console.error('No se encontró el elemento catalogoCerradoOverlay');
            }
            
            resolve(estaAbierto);
        }).catch(error => {
            console.error("Error al verificar estado del catálogo:", error);
            // En caso de error, ocultar el overlay para no bloquear la página
            const overlay = document.getElementById('catalogoCerradoOverlay');
            if (overlay) {
                overlay.style.display = 'none';
            }
            resolve(true); // Asumimos abierto si hay error
        });
    });
}


function monitorearEstadoCatalogo() {
    // Escuchar cambios en tiempo real
    database.ref('config/catalogoEspecial').on('value', (snapshot) => {
        const estaAbierto = snapshot.val() === true;
        const overlay = document.getElementById('catalogoCerradoOverlay');
        
        console.log('Cambio detectado en estado del catálogo:', estaAbierto ? 'ABIERTO' : 'CERRADO');
        
        if (overlay) {
            overlay.style.display = estaAbierto ? 'none' : 'flex';
        }
    });
}

function loadCatalog() {
            database.ref('config/catalogoMensaje').once('value').then((snapshot) => {
                const data = snapshot.val();
                document.getElementById('catalog-title').textContent = data.title;
                document.getElementById('catalog-message').textContent = data.message;
            });
       }
loadCatalog();
...
function cargarProductosFirebase(categoria, contenedor) {
    firebase.database().ref("productos/" + categoria).once("value").then((snapshot) => {
        productosFiltrados = [];
        if (snapshot.exists()) {
            snapshot.forEach((item) => {
                let producto = item.val();
                producto.id = item.key;
                producto.categoria = categoria;
                productosFiltrados.push(producto);
            });
            ordenarPorFavoritos();
        } else {
            contenedor && (contenedor.innerHTML = "<p>No hay productos en esta categoría.</p>");
        }
    }).catch((error) => {
...
document.addEventListener("DOMContentLoaded", (function() {
    console.log("DOM cargado, inicializando..."),
       ...
       loadCatalog();                    
      verificarEstadoCatalogo();
      monitorearEstadoCatalogo();
